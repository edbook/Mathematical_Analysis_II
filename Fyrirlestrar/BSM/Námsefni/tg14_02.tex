...
\begin{equation*}
  ...
  = \lim\limits_{n \to +\infty} \lambda^n |r - x_0|
  = 0,
\end{equation*}
...
\pause
Við munum sýna fram á:  Til er  $\xi_n$ sem liggur í minnsta bilinu
...
\end{equation*}\pause
Eins er $p_n(x_{n+1})=0$ og því
\begin{equation*}
	-p_n(r) = p_n(x_{n+1}) - p_n(r) = -f[x_n,x_{n-1}]e_{n+1}=-f'(\eta_n)e_{n+1},
\end{equation*}
...
Fallið sem hefur snertilinn fyrir graf er 1.~stigs margliðan
\begin{equation*}
...
$x$ og $x_n$  þannig að 
\begin{equation*}
	f(x) - p_n(x) = \frac{1}{2}f''(\xi_n)(x-x_n)^2.
\end{equation*}
}
...
Eins er $p_n(x_{n+1})=0$ og því
\begin{equation*}
	-p_n(r) = p_n(x_{n+1}) - p_n(r) = -f'(x_n)e_{n+1}
\end{equation*}
\pause
Niðurstaðan verður því
\begin{equation*}
